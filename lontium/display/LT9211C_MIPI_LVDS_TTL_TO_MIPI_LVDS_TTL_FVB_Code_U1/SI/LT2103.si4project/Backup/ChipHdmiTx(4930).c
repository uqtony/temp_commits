...
void Chip_HdmiTx_StateHandler(void)
{
    ...
    switch(g_stHdmiTx.ucTxStatus)
    {
        case 0xff:    
            ...
            CHIP_TxHpd_RxCheckSet();
            g_stHdmiTx.ucTxStatus = 0x0a;
            break;
        case 0x0a:
            g_stHdmiTx.ubIsTxEdidRdy = FALSE;
            if(CHIP_RxRdy_GetFlag() == FALSE) //Typec Rx CC is done or dp is ok
            {
                return;
            }
            if(DRV_HdmiTx_HpdDet() == TRUE) //hdmi hpd is high
            {
                ...
                if(CHIP_Rx_DpModeFlag() == TRUE)
                {
                    CHIP_Edid_HdmiToDp(szEdid_Data);
                }
            ...
            break;
        case 0x09: 
            if(CHIP_RxVidRdy_GetFlag()  == TRUE)//rx vide is OK
            {
        ...
        case 0x06:
            ...
            break;
        case 0x05:
            if(CHIP_RxPkgRdy_GetFlag() == TRUE)// dp rx color is ok and rx video ok
            {
