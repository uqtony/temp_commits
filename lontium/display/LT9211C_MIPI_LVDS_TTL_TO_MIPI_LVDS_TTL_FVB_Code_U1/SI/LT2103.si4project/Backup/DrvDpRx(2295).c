...
void DRV_DpRx_Dpcd_Initial(IN u8 ucLinkRate, IN u8 ucLinkCount,IN bool ubSsc_En,IN bool ubTps4_En)
{
    ...
    else
    {
        ...
        DRV_Dpcd_Init_Write(0x02,0x80|ucLinkCount);//c0
        DRV_Dpcd_Init_Write(0x03,0x00|ubSsc_En);
    }
...
void Drv_DpRx_CrLockHoldSet(bool ubIsLock)
{
    if(ubIsLock == TRUE)
    {
        BKDB_0F_REG = 0x00;
...
void DRV_DpRx_PhyCal(void)
{
    ...
    BKE2_44_REG = 0x4c;//rx0 eq stg0 cur/rl ce
    BKE2_4D_REG = 0x4c;//rx1 eq stg0 cur/rl 
    BKE2_56_REG = 0x4c;//rx2 eq stg0 cur/rl 
    BKE2_3B_REG = 0x4c;//rx3 eq stg0 cur/rl 
    
    BKE2_45_REG = 0x4c;//rx0 eq stg1 cur/rl  ce
    BKE2_4E_REG = 0x4c;//rx1 eq stg1 cur/rl 
    BKE2_57_REG = 0x4c;//rx2 eq stg1 cur/rl 
    BKE2_3C_REG = 0x4c;//rx3 eq stg1 cur/rl 
    BKE2_46_REG = 0xcf;//rx0 bf stg0 cur/rl; 
    BKE2_4F_REG = 0xcf;//rx1 bf stg0 cur/rl; 
    BKE2_58_REG = 0xcf;//rx2 bf stg0 cur/rl; 
    BKE2_3D_REG = 0xcf;//rx3 bf stg0 cur/rl;
    BKE2_47_REG = 0xff;//rx0 bf stg1 cur/rl;
    BKE2_50_REG = 0xff;//rx1 bf stg1 cur/rl; 
    BKE2_59_REG = 0xff;//rx2 bf stg1 cur/rl; 
    BKE2_3E_REG = 0xff;//rx3 bf stg1 cur/rl;
    BKE2_48_REG = 0x88;//rx0 eq cs0/cs1;
    BKE2_51_REG = 0x88;//rx1 eq cs0/cs1;
    BKE2_5A_REG = 0x88;//rx2 eq cs0/cs1;
    BKE2_3F_REG = 0x88;//rx3 eq cs0/cs1;
    BKE2_49_REG = 0xf0;//rx0 eq fb0/fb1; 
    BKE2_51_REG = 0xf0;//rx1 eq fb0/fb1; 
    BKE2_5B_REG = 0xf0;//rx2 eq fb0/fb1;
    BKE2_40_REG = 0xf0;//rx3 eq fb0/fb1;
    ucDhRxterm_cnt = 0;
    do
    {
        BKE7_20_REG &= BIT0_0;
...
void DRV_DpRx_PhyCsFbSet(u8 ucLinkRate)
{
    if(ucLinkRate == HBR3)
    {
        BKE2_48_REG = 0xcc;//rx0 eq cs0/cs1;
        BKE2_51_REG = 0xcc;//rx1 eq cs0/cs1;
        BKE2_5A_REG = 0xcc;//rx2 eq cs0/cs1;
        BKE2_3F_REG = 0xcc;//rx3 eq cs0/cs1;
        BKE2_49_REG = 0xc0;//rx0 eq fb0/fb1; 
        BKE2_51_REG = 0xc0;//rx1 eq fb0/fb1; 
        BKE2_5B_REG = 0xc0;//rx2 eq fb0/fb1;
        BKE2_40_REG = 0xc0;//rx3 eq fb0/fb1;
    }
    else
    {
        BKE2_48_REG = 0x88;//rx0 eq cs0/cs1;
...
{
    BKDB_C2_REG = 0x2a;//Link lock soft mode Link lock status in soft mode/Eq stage select in tps1 pattern
...
{
    BKDB_B5_REG &= 0xfb;////DPRX mainlink data soft reset.
    BKDB_B5_REG |= 0x04;///DPRX mainlink data soft reset.
}
...
void DRV_DpRx_PcrIni(void)
{
    ...
    BKDA_87_REG = 0Xf8;//Dividing ratio upper limit threshold
...
{
    DRV_DpRx_Dpcd_Initial(g_stDpRx.ucRxLinkRate,g_stDpRx.ucRxLinkCount,g_stDpRx.ubSsc_Support,g_stDpRx.ubTps4_Support);
