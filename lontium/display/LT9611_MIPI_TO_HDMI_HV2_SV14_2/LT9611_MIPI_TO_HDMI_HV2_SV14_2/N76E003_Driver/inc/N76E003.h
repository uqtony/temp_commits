...
--------------------------------------------------------------------------*/
/*
51系列单片机内部主要有四大功能模块，分别是I/O口模块、中断模块、定时器模块和
串口通信模块(串行I/O口)

51单片机内部共有21个SFR，其布局如图2，从图中可以看出，每个SFR占1个字节，
多数字节单元中的每一位又有专用的“位名称”。这21个SFR又按是否可以位寻址
分为两大部分，ACC、IE、P1等11个可以位寻址，SP、TMOD等不可以位寻址。
*/
sfr P0			= 0x80;
sfr SP			= 0x81;
sfr DPL			= 0x82;
